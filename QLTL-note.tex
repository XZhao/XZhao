...
Theorem (Ultimately Periodic Model Theorem): A LTL formula $f$ is satisfiable. Then it has a model $\xi$ which is ultimately periodic with starting index $l\leq 2^{1+length(f)}$ period $p\leq 4^{1+length(f)$ and for all $l\geq l$, $S[\xi(k),f]=S[\xi(k+p),f]$, and every $\xtsf{U}$ formula in $S[\xi(k),f]$ is fulfilled before $k+p$. 

Proof: Let $\pi$ be a model of $f$. Then $\pi(0)\models f$
