...
\begin{document}
\author{Hans Kleine Büning \inst{1}, Yuhan Yan\inst{1} and Xishun Zhao\inst{2}
\thanks{Partially supported by NSFC Grant 61272059 and MOE grant 11JD720020.}
}

\institute{Computer Science Institute, University of Paderborn, Germany\\
\email{kbcsl@upb.de} (H. Kleine Büning), \email{yuhan@upb.de} (Y. Yan)
\and
...
\cite{KBLettmann}. We will investigate the configuration problems for various classes: CNF is the classs of formulas in conjunctive normal form, DNF are the formulas in disjunctive normal form, HORN is the set of Horn formulas, and DHORN is the set of definite Horn formulas (Horn formulas without negative clauses).
...
$N_i$ is the names of the component $\alpha_i$. The demand that if $\alpha_3$ is in the solution then $\alpha_4$ belongs to the solution, too, can be define by $N_3 \rightarrow N_4$. These formulas can be considered as global knowledge.\\
...
%
We say a subset $K \subseteq C$ satisfies $\Delta$ if $\Delta$ is true for the truth assignment $v$ defined by 
...
each components $\alpha_i$ has the name $N_i$, and the global knowledge $\Delta$ \\ 
& is a formula over $N_1, \ldots, N_n$.\\
{\em Query:} & $\exists K \subseteq C: K \in \mbox{ SAT}, K \mbox{ satisfies }\Delta, K \models \beta$.\end{tabular}

\noindent We can transform a configuration problem with global knowledge into a problem without global knowledge in polynomial time as follows:
We associate to $C$ and $\Delta$ the following set of components:
$$C_{ext}=\left\{\alpha_i \wedge N_i \wedge \Delta \wedge \bigwedge_{1 \leq j\not = i \leq n}
(N_j \rightarrow \alpha_j) : 1 \leq i \leq n\right\}.$$
...
\begin{lemma}
Let $C=\{\alpha_1, \ldots, \alpha_n\}$ be a set of components, where $N_i$ is the name of $\alpha_i$. The global knowledge $\Delta$ is a formula over $N_1, \ldots, N_n$ and $\beta$ is the target formula. For the associated set of components $C_{ext}$ the folowing two statements are equivalent:

(1) $\exists K \subseteq C: K \in \mbox{SAT, } K \mbox{ satisfies } \Delta, K \models \beta$.
  
(2) $\exists K' \subseteq C_{ext}: K' \in \mbox{SAT and } (\exists N_1 \ldots \exists N_n K') \models \beta$.
\end{lemma}

Next we introduce more formally the configuration problems. 

\begin{definition}(Consequence Configuration Problem for $\models$, \AM, \AMB)\\
 Let $R$ and $T$ be classes of propositional formulas. Then we define\\
the following problems $(R, \models, T), (R, \AM, T)$ and $(R, \AMB, T)$: 

\begin{tabular}{ll}
{\em Instance} $:$ & \hskip -2cm A set of components $C=\{\alpha_1,\cdots,\alpha_n\} \subseteq R$, a target\\ & \hskip -2cm formula $\beta \in T$, and sets of variables $X$ and $Y$.\\
{\em Query for} $(R, \models, T):$ & $\exists K \subseteq C: K \in$ SAT and $K \models \beta$?\\
{\em Query for} $(R, \AM, T):$ & $\exists K \subseteq C: K \in$ SAT and $(\exists X K) \models \beta$?\\
{\em Query for} $(R,  \AMB,  T):$ & $\exists K \subseteq C: K \in$ SAT and $(\exists X K) \models (\exists Y \beta)$?
\end{tabular}
\end{definition}

\begin{definition}(Equivalence Configuration Problem for $\models$, \AEQ, \AEQB)\\
 Let $R$ and $T$ be classes of propositional formulas. Then we define\\
the following problems $(R,  \approx, T)$, $(R, \AEQ, T)$ and $(R, \AEQB, T)$: 

\begin{tabular}{ll}
{\em Instance} $:$ & \hskip -2cm A set of components $C=\{\alpha_1,\cdots,\alpha_n\} \subseteq R$, a target formula \\ &\hskip -2cm $\beta \in T$, and sets of variables $X$ and $Y$.\\
{\em Query for} $(R, \approx, T):$ & $\exists K \subseteq C: K \in$ SAT and $K \approx \beta$?\\
{\em Query for} $(R,\AEQ, T):$ &  $\exists K \subseteq C: K \in$ SAT and $(\exists X K) \approx \beta$?\\
{\em Query for} $(R, \AEQB, T):$ &  $\exists K \subseteq C: K \in$ SAT and $(\exists X K) \approx(\exists Y \beta)$?
\end{tabular}
\end{definition}
...
the problem (DHORN $\leq k$, $\models$, DHORN) is the configuration problem, for which the components and the target formula are definite Horn formulas and the configuration has at most $k$ components. Likewise for (DHORN,\AM, DHORN) and (DHORN, \AMB, DHORN). 

\begin{theorem}
The table contains the computational complexities of the configuration problem for $\models$, \AM, and \AMB.\\

\begin{tabular}{|l|l|l|l|l|l|}
\hline
 & Class $R$  & Class $T$ &  $(R,\models,T)$ & $(R, \AM, T)$ & $(R, \AMB, T)$ \\ \hline
1. & DHORN & DHORN          & PTIME & PTIME  & coNP-c         \\ \hline
2. & DHORN $\leq k$ & DHORN & NP-c  & NP-c   & $\Sigma^P_2$        \\ \hline
3. & 2-Term & Term          & NP-c  & NP-c   & NP-c           \\ \hline
4. & HORN & HORN                       & NP-c  & NP-c   & $\Sigma^p_2$-c \\ \hline
5. & CLAUSE & LITERAL       & $\Sigma_2^P$-c & $\Sigma_2^P$-c & $\Sigma_2^P$-c \\ \hline
6. & CNF & CLAUSE  & $\Sigma_2^P$-c & $\Sigma_2^P$-c & $\Sigma_2^P$-c \\ \hline
7. & CNF & CNF              & $\Sigma_2^P$-c & $\Sigma_2^P$-c & $\Sigma_3^P$ \\ \hline
\end{tabular}
\end{theorem}
...
\begin{proof}
Ad 1: (DHORN, $\models$, DHORN) and (DHORN, \AM, DHORN): Since every definite Horn formula is satisfiable, we only we have to test whether for the complete set of components $C$ we have $C \models \beta$ or $\exists X C \models \beta$. That can be performed in polynomial time \cite{Doga}.

(DHORN,\AMB, DHORN): For definite Horn formulas the problem whether $\exists K \subseteq C: (\exists X K) \models \exists Y \beta)$ is equivalent to the problem of deciding whether$(\exists X C) \models (\exists Y \beta)$. This problem is coNP-complete \cite{KBLettmann}\\
...
Since $K \models \beta$ (resp. ($\exists X K) \models \beta$) is decidable in polynomial time, the configuration problem is in NP (guess a subset $K \subseteq C$).
...
AD 3:  (2-Term,$\models$, (\AM, \AMB resp.), Term)): For terms, existential quantifiers simplify the term to true, if the variable occurs as literal, or the quantifier variable can be omitted. Thus, it suffices to investigate the case (2-Term, $\models$, Term).
...
Remark: Additionally, we have $\exists X K \approx \beta$ for $X = var(\alpha)$.
\\

Ad 4: (HORN, $\models$, HORN) and (HORN, \AM HORN):\\
...
(HORN, \AMB, HORN):  That the problems is in $\Sigma^P_2$ is obvious. We can guess some satisfiable $K \subseteq C$ and
then decide the coNP-problem $(\exists X K) \models (\exists Y \beta)$. For the complementary problem guess a
...
AD 5,6, and 7:
At first, we shall show that the problem (CLAUSE, $\models$, Literal) is $\Sigma^P_2$-hard. Then we will prove that the configuration problems (CNF, \AMB, CLAUSE) and (CNF, \AM, CNF) belong to $\Sigma^P_2$. That shows that all problems, except for (CNF, \AMB, CNF), are $\Sigma^P_2$-complete.
Finally we shall investigate the case (CNF. \AMB, CNF).

Part 1. (CLAUSE, $\models$, Literal) is $\Sigma^P_2$-hard:\\
...
Part 2. (CNF, \AMB, CLAUSE) and (CNF, \AM, CNF) are in $\Sigma^P_2$. 
Since an existentially quantified clause evaluates to true or to a clause, it suffices to consider the case that the target formula $\beta$ does contain variables of $X$. Then we have:
...
\begin{theorem}
The table contains the computational complexities of the configuration problem for
$\approx$, \AEQ, and \AEQB. 
\vskip 2mm

\begin{tabular}{|l|l|l|l|l|l|}
\hline
 & {\em Class $R$} &  {\em Class $T$} & {$(R, \approx, T)$} & {$(R, \AEQ, T)$} & 
{$(R,\AEQB, T)$ }\\ \hline
...
have: $\exists K \subseteq C: K \approx \beta$ iff $F(\beta) \approx \beta$. Thus, it remains to decide whether $F(\beta) \models \beta$. Because $F(\beta)$ and $\beta$ are Horn formulas, that can be performed in polynomial time.\\
...
AD 3: (HORN,  $\approx$, HORN): Let $C=\{\alpha_1, \ldots, \alpha_n\}$ be the set of components and $\beta$ be the target formula. The test whether the target formula is satisfiable costs at most linear time. If the target formula is unsatisfiable then
...
(HORN, \AEQ, HORN) and (HORN, \AEQB, HORN): 
...
which is coNP-complete \cite{KBLettmann}. The first part to select a satisfiable subset $K$ is in NP. 
...
For each variable $z$, we introduce a new variable $\pi(\neg z)$ and define $\pi(z)=z$.

%Furthermore, we define $\psi_1:=\left(\bigvee_{i=1}^m (\neg x_i\vee \neg g(\neg x_i))\right)$.
%Then, 
%
%$\Phi$ is true iff $\exists \pi(y_1),\cdots, \pi(y_k) \forall x_1,\cdots, x_m (\pi(\phi) %\wedge \psi_1)$ is true.
%
%\noindent 
%Let $$\psi_0:=\left(\bigwedge_{j=1}^m (\neg \pi(\neg x_j)\vee \neg x_j)\right) \mbox{ and %}\theta:=\left(\bigvee_{i=1}^m (\neg x_i\wedge \neg\pi(\neg x_i))\right)$$
%

\noindent Let $$\psi_0:=\left(\bigwedge_{j=1}^m (\neg \pi(\neg x_j)\vee \neg x_j)\right), \ \psi_1:= \left(\bigwedge_{i=1}^k(\neg \pi(\neg y_i)\vee \neg y_i)\right)$$
...
$$\theta \approx \exists a_0 \ldots \exists a_k \left(\neg a_k \wedge a_0 \wedge \bigwedge_{1 \leq i \leq k-1} \left((\neg a_i \vee \neg x_i \vee a_{i+1}) \wedge (\neg a_i \vee \neg \pi(\neg x_i) \vee a_{i+1})\right)\right).$$


\noindent For a conjunction $c=L_1\wedge\cdots\wedge L_s$, we write 
$\pi(c):=\pi(L_1)\wedge\cdots\wedge \pi(L_s)$. For a new variable $U$ we associate to $\Phi$ the set of components $C$:


$$\begin{array}{lcl}C_0&:=&\{\psi_0\wedge\psi_1 \wedge\left( \bigwedge_{i=1}^n(\pi(c_i)\rightarrow \neg U\vee \theta)\right)\}\\ 
C&:=&C_0\cup \{y_i,\pi(\neg y_i)\mid i=1,\cdots,k\}\\
%\beta&:= &(\neg U\vee\theta)\wedge \psi_0 \\
%V&:=&\{U\}\cup\{x_1,\cdots,x_m,\pi(\neg x_1),\cdots, \pi(\neg x_m)\}
\end{array}$$

\noindent The target formula is
$\beta := (\neg U\vee\theta)\wedge \psi_0$. 
%
The sets of helping variables are  $Y := \{y_1, \ldots y_k, a_0, \ldots, a_k, \pi(\neg y_1), \ldots, \pi(\neg y_k)\}$ and $A := \{a_0, \ldots, a_k\}$.
%
The set of free variables is $V:=\{U\}\cup\{x_1,\cdots, x_m,\pi(\neg x_1),\cdots, \pi(\neg x_m)\}$.
...
%
$$K := C_0\cup \{y_i: v(y_i)=1, 1\leq i\leq k\}\cup \{\pi(\neg y_i): v(y_i)=0, 1\leq i\leq k\}$$
%
...
Then we have
$$\begin{array}{ll}\exists X K \approx z & 
\Longrightarrow 
...
\mbox{ is true}\\
&\Longrightarrow$ $\exists X \forall Y \phi\mbox{ is true}.\end{array}$$
...
we obtain:
$$\begin{array}{ll}\exists X K & \approx \bigwedge_{1 \leq j \leq r} (\neg \phi_j \vee z)[x_1/\epsilon_1, \ldots, x_n/\epsilon_n]\\
...
&\approx \neg (\exists X \forall Y \phi) \vee z\approx z\end{array}$$

\vskip 2mm
Ad 5: (CNF, $\approx$, CNF): 
For this configuration problem we have no completeness result. Clearly, the upper bound is $P^{NP[\text{log}\, n]}$.
The $D^P$-hardness can bee seen by a reduction from the minimal unsatisfiability problem for CNF formulas, which is complete for this complexity class: 
%%%%%%
...
(CNF, \AEQ, CNF) and (CNF, \AEQB, CNF): 
We shall show that they are in $\Sigma^P_3$ and that the configuration problems are $\Sigma^p_3$-hard, even for target formulas given as terms.
%%%%%%%%%%%
